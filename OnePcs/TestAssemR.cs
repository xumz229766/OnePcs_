...
namespace _OnePcs
{
    public class TestAssemR : AssemRModule
    {
        private string strOut = ",右组装验证模块,Action,";
                ...
                else
                {
                    switch (action)
                    {
                        case ActionName._50动作选择:
                            ...
                            if (bUseCamCenter)
                            {
                                AssemRModule.dAssemCenterRow = 1024;
                                AssemRModule.dAssemCenterCol = 1224;
                        ...
                        case ActionName._50选择结果:
                            BarrelModule.iCamTimes = 0;
                            AssemRModule.dAssemCenterRow = ModelManager.BarrelParam.imgResultUp.CenterRow;
                            AssemRModule.dAssemCenterCol = ModelManager.BarrelParam.imgResultUp.CenterColumn;
                            step = step + 1;
                            break;
