...
namespace Assembly
{
    public class GetProduct1Module:ActionModule
    {
        private string strOut = "GetProduct1Modul-Action-";
        ...
        public GetProduct1Module()
        {
            ...
            lstAction.Add(ActionName._10Z轴到取料运行位);
            lstAction.Add(ActionName._10Z轴到位);
        ...
        //public static GetProduct1Module GetInStance()
        //{
        //    if (module == null)
        //        module = new GetProduct1Module();
        //    return module;
        //}
        public override void Reset()
        {
            switch (iResetStep)
            {
                ...
                case 1:
                    ...
                    {
                        if (mc.dic_Axis[AXIS.取料X1轴].INP && mc.dic_Axis[AXIS.取料Y1轴].INP && mc.dic_Axis[AXIS.取料Z1轴].INP)
            ...
            try
            {
                ...
                switch (action)
                {
                    case ActionName._10判断取料盘是否为空:
                        if (GetState1 == ActionState.空闲 || GetState1 == ActionState.取料)
                        {
                            ...
                            if (IsBarrelEmpty())
                            {
                                ...
                                Alarminfo.AddAlarm(Alarm.取料1有空盘);
                                Run.runMode = RunMode.暂停;
                                WriteOutputInfo(strOut + "取料1盘空，停止取料！");
                                break;
                            }
                            ...
                            if (iCurrentBarrel > 0)
                            {
                                ...
                                {
                                    if ((Run.assem1Module.IStep > Run.assem1Module.lstAction.IndexOf(ActionName._30开始组装) + 1) || (iCurrentBarrel < CommonSet.dic_BarrelSuction[1].EndPos) || (lstNGSuction.Count > 0))
                                    {
                                        step = step + 1;
                                        CommonSet.swCircleA1.Restart();
                                        break;
                                    }
                                    //如果最后收尾组装飞拍NG后抛料
                                    if ((!Assem1Module.bImageResult) && (Run.assem1Module.IStep > Run.assem1Module.lstAction.IndexOf(ActionName._30开始抛料)))
                                    {
                                        step = step + 1;
                                        CommonSet.swCircleA1.Restart();
                                        break;
                    ...
                    case ActionName._10计算吸笔XY坐标:
                        //如果使用相机拍照
                        if (CommonSet.dic_OptSuction1[iCurrentSuction].BUseCameraUp)
                        {
                            OptSution1 suction = CommonSet.dic_OptSuction1[iCurrentSuction];
                            currentPoint = suction.getCameraCoordinateByIndex();
                            currentPoint.Z = CommonSet.dic_OptSuction1[iCurrentSuction].DGetPosZ;
                            step = step + 1;
                        }
                        else
                        {
                           ...
                           // currentPoint.Z = OptSution1.pSafeXYZ.Z;
                            step = lstAction.IndexOf(ActionName._10Z轴到取料运行位);
                        }
                        break;
                    case ActionName._10上相机拍照:
                        ...
                        if (sw.WaitSetTime(10))
                        {
                            ...
                            WriteOutputInfo("取料1上相机拍照");
                            step = step + 1;
                        }
                        break;
                    case ActionName._10上相机拍照完成:
                        if (CommonSet.dic_OptSuction1[iCurrentSuction].imgResultUp.bStatus)
                        {
                            if (CommonSet.dic_OptSuction1[iCurrentSuction].imgResultUp.bImageResult)
                            {
                            ...
                            {
                                CommonSet.dic_OptSuction1[iCurrentSuction].CurrentPos++;
                                step = lstAction.IndexOf(ActionName._10计算吸笔XY坐标);
                            }
                        }
                        else
                        {
                            Alarminfo.AddAlarm(Alarm.取料上相机1拍照超时);
                            WriteOutputInfo(strOut + "取料上相机1拍照超时");
                           
                        }
                        break;
                    case ActionName._10XY轴到拍照位:
                        if ((mc.dic_Axis[AXIS.取料Z1轴].dPos < (currentPoint.Z - 3)) || IsAxisINP(dDestPosZ, AXIS.取料Z1轴, 1))
                        {
                    ...
                    case ActionName._10XY到安全位:
                        ...
                        mc.AbsMove(AXIS.取料X1轴, dDestPosX, (int)100);
                        mc.AbsMove(AXIS.取料Y1轴, dDestPosY, (int)100);
                    ...
                    case ActionName._10XY到位完成:
                        ...
                        {
                            WriteOutputInfo(strOut + "取料XY到位完成");
                            step = step + 1;
                        }
                        break;
                    case ActionName._10Z到取料位:
                        if (!mc.dic_Axis[AXIS.取料Z1轴].INP)
                            break;
                        if (CommonSet.bTest)
                        {
                        ...
                        WriteOutputInfo(strOut + "取料Z1轴到取料位：" + dDestPosZ.ToString());
                        step = step + 1;
                    ...
                    case ActionName._10Z轴到取料运行位:
                        ...
                        {
                            int iFirst = 9;//第一个吸嘴，不用到运行位 
                            double min = 100;
                            foreach (KeyValuePair<int, OptSution1> pair in CommonSet.dic_OptSuction1)
                            {
                                if (pair.Value.BUse)
                                {
                                    if (pair.Value.DGetPosZ < min)
                                    {
                                        min = pair.Value.DGetPosZ;
                                    }
                                    if (pair.Key < iFirst)
                                    {
                                        iFirst = pair.Key;
                            ...
                            dDestPosZ = min-5;
                            mc.AbsMove(AXIS.取料Z1轴, dDestPosZ, (int)CommonSet.dVelRunZ);
                            WriteOutputInfo(strOut + "Z轴到取料运行位：" + dDestPosZ.ToString());
                            step = step + 1;
                        }
                    ...
                    case ActionName._10Z轴到待机位:
                        dDestPosZ = OptSution1.pSafeXYZ.Z;
                        mc.AbsMove(AXIS.取料Z1轴, dDestPosZ, (int)50);
                        WriteOutputInfo(strOut + "Z轴到安全位：" + dDestPosZ.ToString());
                        step = step + 1;

                        break;
                    case ActionName._10Z轴到位:
                        if (IsAxisINP(dDestPosZ, AXIS.取料Z1轴, 1))
                        {
                            WriteOutputInfo(strOut + "取料Z1轴到位完成");
                            step = step + 1;
                        }
                    ...
                    case ActionName._10吸笔吸真空:
                        ...
                        long lTime1 = (long)(CommonSet.dic_OptSuction1[iCurrentSuction].DGetTime * 1000);
                            ...
                            if (iCurrentTimes > CommonSet.iGetTimes - 1)
                            {
                                if (iCurrentNum > CommonSet.iTimes - 1)
                                {
                                    if (Run.runMode == RunMode.运行)
                                    {
                                        ...
                                        CommonSet.AlarmProcessShow(AlarmBlock.取料1, step, step-iIndex, "取料吸笔" + iCurrentSuction.ToString() + "连续吸取NG", true);
                                        Run.runMode = RunMode.暂停;
                                    }
                                ...
                                else
                                {
                                    ...
                                    CommonSet.dic_OptSuction1[iCurrentSuction].CurrentPos++;
                                    //如果盘子无料
                                    if (CommonSet.dic_OptSuction1[iCurrentSuction].bFinish)
                                    {
        ...
        public int FindSuctionEmpty()
        {
            ...
            for (; result < 10; result++)
            {
                ...
                OptSution1 p = CommonSet.dic_OptSuction1[result];
